...
module Codegen =
    ...
    module Message =
        ...
        let private genDecodeForSignal (s: Signal) (doRangeCheck: bool) (config: Signal.CANdy.Core.Config.Config) =
            ...
            let physAssignFloatDouble = sprintf "    msg->%s = (float)((double)%s * %.17g + %.17g);" s.Name rawCast s.Factor s.Offset
            let physAssignFloatFloat = sprintf "    msg->%s = (float)(((float)%s * (float)%.17g) + (float)%.17g);" s.Name rawCast s.Factor s.Offset
            let physAssign =
                ...
                | "fixed" ->
                    ...
                    | Some scale when abs (s.Offset - Math.Round(s.Offset)) < 1e-12 ->
                        sprintf "    msg->%s = (float)(((double)%s + (%.0f)) / (double)%d);" s.Name rawCast (Math.Round(s.Offset * (float scale))) scale
