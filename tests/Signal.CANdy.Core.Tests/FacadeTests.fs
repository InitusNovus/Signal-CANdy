...
"""
    ...
    let ``ParseDbc throws SignalCandyParseException for non-existent file`` () =
        let facade = GeneratorFacade()
        let ex = Assert.Throws<SignalCandyParseException>(fun () ->
            facade.ParseDbc("absolutely_nonexistent.dbc") |> ignore)
    ...
    let ``ValidateConfig throws SignalCandyValidationException for invalid config`` () =
        ...
        let badConfig : Signal.CANdy.Core.Config.Config =
            { PhysType = "INVALID"
              ...
              FilePrefix = "sc_" }
        let ex = Assert.Throws<SignalCandyValidationException>(fun () ->
            facade.ValidateConfig(badConfig))
